...
- 数据看板：在当前浏览器中统计各页面访问次数、玩法详情浏览次数与收藏动作，提供浏览排行、收藏排行、页面访问分布及 CSV 导出。统计存放在 `localStorage`，不会上传，也不代表全站用户数据。
...
GitHub Pages 没有可写入的数据库，因此数据看板默认只统计当前浏览器。若要展示全站总浏览量和所有访客的玩法热度，需要另接可写入的统计服务或数据库；前端排行榜结构可以继续复用。
