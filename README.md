...
截至 2026-09-19，玩法库收录 498 个模式案例、14 条基础更新，覆盖 7 款游戏；Free Fire 另有 70 篇版本及活动详录、16 篇关联公告，不计为新增模式，历史覆盖仍待补齐。地图页现收录三角洲行动 20 张、战地风云 6 16 张、无畏契约 13 张、PUBG 10 张和 Apex Legends 6 张地图。原有 6 张玩法机制卡和 6 张关卡机制卡暂存本地，不加载、不公开发布。
...
- 地图：按游戏归档，支持三角洲行动、战地风云 6、无畏契约、PUBG 和 Apex Legends 筛选，支持中文搜索、放大预览、单图下载和来源跳转。三角洲行动保留本地互动地图入口，可筛选点位、搜索定位、切换楼层与战场配置；战地风云 6 已接入 9 张公开模式战术图，无畏契约已接入 13 张游戏内小地图 / Callout 平面图，PUBG 已接入 10 张 PUBG Planet 2D 平面图，Apex Legends 已接入 6 张 Apex Legends Wiki 地图平面图。
- 玩法机制、关卡机制：暂停公开；原稿保存在被 Git 忽略的 `local-only/research.json`。旧栏目地址回到模式首页。
- 玩法模式内置三个视图：案例库、玩法动态和时间画板。玩法动态按案例上线时间排列，提供近 30 天、即将上线、LTM 限时模式筛选；时间画板按游戏与六种核心玩法分轨，LTM 标在节点上，与同类核心模式共用轨道。
- 我的收藏：兼容原有浏览器收藏标识，支持玩法模式收藏；既有草稿收藏标识保留在浏览器，但不显示或导出未公开内容。
- 站点在正式 GitHub Pages 域名下把匿名计数事件发送到 Cloudflare Worker，供站长通过私有数据回收 Skill 离线查看全站趋势。前端不提供数据看板；D1 只保存随机浏览器标识的哈希、页面或玩法编号与时间，不保存 IP、账号、设备信息或来源页。

详情支持 Markdown 复制、PNG 分享与来源跳转；分享弹窗和 PNG 卡片同步研究工作台的浅色绿色 UI。顶部可导出当前筛选的 Obsidian Canvas，或收藏的 Markdown 合集。支持分页加载、空结果提示、缺图回退、移动端导航、键盘操作与详情深链接。
...
- app.js：日期处理、游戏时间画板、收藏、匿名计数上报与导出/分享工具。
...
GitHub Pages 没有可写入的数据库。全站匿名事件经 `workers.dev` 上的 Cloudflare Worker 写入私有 D1 数据库；页面不显示统计数据，站长通过本机私有数据回收 Skill 获取汇总和备份。Worker 源码、D1 表结构和管理密钥只保存在被 Git 忽略的 `local-only/` 与 Cloudflare Secrets 中。
...
核查日期：2026-09-06。三角洲行动来源为[国服官方地图工具](https://df.qq.com/cp/a20240729directory/)，当前归档覆盖 6 张烽火地带地表图与 14 张全面战场 PC 攻防底图；黑鹰坠落战役暂未找到公开底图。战地风云 6 使用 [EA 官方地图页](https://www.ea.com/games/battlefield/battlefield-6/features/maps)的 16 张地图视觉图，无畏契约使用 [Riot 官方地图页](https://playvalorant.com/en-us/maps/)的 13 张地图视觉图。PUBG 使用 [PUBG 官方地图页](https://pubg.com/en/game-info/maps/taego)核对地图清单，并归档 [PUBG Planet](https://pubgplanet.com/maps) 的 10 张 2D 地图平面图；Apex Legends 使用 [EA 官方地图中心](https://www.ea.com/games/apex-legends/apex-legends/game-objects/maps-hub)核对 6 张 BR 地图清单，并归档 [Apex Legends Wiki 地图图像](https://apexlegends.wiki.gg/wiki/Category:Map_images)。地图名称保留中文并附英文原名。

公开 2D 平面图另外记录在 `planImageUrl`：无畏契约 13 张使用 VALDB / Valorant API 的游戏内小地图，战地风云 6 9 张使用 Battlefield6.gg 的模式战术图，PUBG 10 张使用 PUBG Planet 的烘焙地形与点位图，Apex Legends 6 张使用 Apex Legends Wiki 的 2048 级地图图像。卡片会标记图的类型与来源，不把静态平面图误称为完整互动地图。

地图记录还保留 `wikiUrl`：战地风云 6 与无畏契约跳转到对应地图 Wiki，三角洲行动跳转到 Delta Force Wiki 的地图索引；详情弹窗与卡片都提供 `Wiki ↗` 入口。无畏契约额外保存 `planCalloutPoints`，把 VALDB 的点位名称和百分比坐标直接叠加在地图上。
...
自 2026-09-16 起，采集范围同时包含「基础局内体验更新」：地图/POI、地图交互物、装备道具、移动转点、资源经济、救援返场、战斗规则和局内操作反馈。即使没有新模式名称，也需检查并整理。详见 [基础体验更新收集口径](docs/core-experience-updates.md) 和 [FF 首批清单](docs/free-fire-core-updates-2026-09-16.md)。资料保存在 `data/core-updates.json`，独立覆盖账本为 `data/core-update-source-coverage.json`。2026-09-18 已接入案例库、动态和时间轴，通过资料类型和更新类别筛选；基础更新与模式案例分别计数。图片归档在 `assets/core-updates/`，来源和文件校验记录在 `data/core-update-image-sources.json`。运行 `node scripts/validate_core_updates.cjs` 检查图片、日期、筛选、分组与导出。
...
图片 URL 应该是可直接访问的 CDN 地址，不加查询参数，除非确实需要缓存控制；已核实的官方素材也可以归档到 `assets/modes/`，此时在记录中保留 `imageAssetSourceUrl` 追溯原始 CDN。无法确认来源时，不要用作正式封面图。
...
### Free Fire BR / CS 版本详录

[当前版本详录](docs/free-fire-patches/README.md) 已按 BR、CS、通用局内调整拆分。案例库保留原有模式/基础更新卡，并提供版本详录入口；时间画板有 BR/CS 版本两线。各版分类小节、参数表和公告正文插图保存在 `data/free-fire-patches.json` 与 `assets/ff-patches/`。其他玩法及局外章节为目录摘要，不代表连续历史覆盖。

版本详录将官方图片直接嵌入有关小节，总览和其他章节图片也直接展示，保留原比例与原图链接。BR/CS 筛选后的剩余图片仍在“其余原文配图”区域可见；Markdown 同步嵌图。后续所有 FF 公告必须逐一核对正文图片清单，不能只存文件或放入默认折叠图库。

更新详录后运行 `python3 scripts/build_ff_patch_docs.py`、`node scripts/validate_ff_patches.cjs`，再执行原有 core/taxonomy 校验和公开构建。本地修改不自动部署。
