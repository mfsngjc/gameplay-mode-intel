...
截至 2026-09-07，玩法库收录 496 个模式案例，覆盖 7 款游戏；地图页现收录三角洲行动 20 张、战地风云 6 16 张、无畏契约 13 张、PUBG 10 张和 Apex Legends 6 张地图。原有 6 张玩法机制卡和 6 张关卡机制卡暂存本地，不加载、不公开发布。
...
- 数据看板：页面内仍展示当前浏览器的访问、玩法浏览与收藏统计；同时把匿名计数事件发送到 Cloudflare Worker，供站长离线汇总全站趋势。D1 只保存随机浏览器标识的哈希、页面或玩法编号与时间，不保存 IP、账号、设备信息或来源页。
...
- workspace.js：导航、搜索、过滤、卡片、详情、资料关联、统计看板与响应式交互。
- app.js：日期处理、游戏时间画板、收藏、本机访问统计与导出/分享工具。
...
GitHub Pages 没有可写入的数据库，因此页面内的数据看板仍只显示当前浏览器。全站匿名事件经 `workers.dev` 上的 Cloudflare Worker 写入私有 D1 数据库；Worker 源码、D1 表结构和管理密钥只保存在被 Git 忽略的 `local-only/` 与 Cloudflare Secrets 中。
